...
class Lexer:
    ...
    def read_preprocessor(self) -> Token:
        ...
        directive = ''
        while self.pos < len(self.source) and self.current().isalpha():
            directive += self.advance()

        if directive not in PREPROCESSOR_DIRECTIVES:
            self.errors.append(LexError(
                line=start_line,
                error_type='INVALID_PREPROCESSOR',
                detail=f'Неизвестная директива: «#{directive}»'
            ))
        ...
        return Token('PREPROCESSOR', f'#{directive}{rest}', start_line)
